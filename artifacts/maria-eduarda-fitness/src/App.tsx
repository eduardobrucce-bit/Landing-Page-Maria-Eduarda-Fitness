    ...
    <header className="sticky top-0 z-40 border-b border-[hsl(var(--foreground)/.08)] bg-[hsl(var(--background)/.88)] backdrop-blur-xl">
      <div className="container-page flex h-[76px] items-center justify-between">
        <button data-testid="button-brand" onClick={() => goTo('inicio')} className="flex items-center gap-2 text-left">
          <span className="grid h-9 w-9 place-items-center rounded-full bg-[hsl(var(--primary))] text-sm font-bold text-white">ME</span>
          <span className="leading-none"><strong className="block text-[15px] tracking-[-.02em]">{SITE_CONFIG.specialistName}</strong><small className="mono text-[8px] uppercase tracking-[.14em] text-[hsl(var(--muted-foreground))]">fitness & rotina</small></span>
        </button>
...
function Testimonials() {
  return <section className="section-pad bg-[hsl(var(--foreground))] text-[hsl(var(--background))]"><div className="container-page"><div className="flex flex-col justify-between gap-8 md:flex-row md:items-end"><div><div className="eyebrow mb-5 text-[hsl(var(--accent))]">prova social real</div><h2 className="text-5xl leading-[.94] tracking-[-.05em] md:text-7xl">Resultados que vão<br /><em className="serif font-normal text-[hsl(var(--accent))]">além da balança.</em></h2></div><p className="max-w-[250px] text-xs leading-5 text-[hsl(var(--background)/.58)]">Os relatos abaixo são espaços editáveis para inserir experiências reais de clientes.</p></div><div className="mt-16 grid gap-4 md:grid-cols-3">{SITE_CONFIG.testimonials.map((testimonial, i) => <article data-testid={`card-testimonial-${i + 1}`} key={testimonial.name} className="relative rounded-2xl border border-[hsl(var(--background)/.16)] p-6 transition-transform duration-300 hover:-translate-y-2"><Quote size={25} className="text-[hsl(var(--accent))]" /><p className="mt-8 min-h-[96px] text-sm leading-6 text-[hsl(var(--background)/.76)]">{testimonial.text}</p><div className="mt-7 flex items-center gap-3 border-t border-[hsl(var(--background)/.13)] pt-4"><div className="grid h-9 w-9 place-items-center rounded-full bg-[hsl(var(--secondary))] text-xs font-bold text-[hsl(var(--primary))]">{testimonial.name.replace(/[^A-Za-zÀ-ÿ]/g, '').slice(1, 3).toUpperCase() || 'CL'}</div><div><strong className="block text-xs">{testimonial.name}</strong><span className="text-[11px] tracking-[.16em] text-[hsl(var(--accent))]">★★★★★</span></div></div></article>)}</div><p className="mt-7 text-center text-[11px] text-[hsl(var(--background)/.45)]">Resultados individuais podem variar. <span className="underline">Substitua os placeholders por depoimentos autorizados.</span></p></div></section>;
}
